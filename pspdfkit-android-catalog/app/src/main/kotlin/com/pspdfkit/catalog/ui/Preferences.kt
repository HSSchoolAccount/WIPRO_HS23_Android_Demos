        ...
        withContext(Dispatchers.IO) {
            ...
            preferences =
                if (stringToMatch.isBlank()) {
                    state.preferenceSections
                } else {
                    state.preferenceSections.mapNotNull { section ->
                        // If the section name matches, everything inside of it should match
                        if (section.title.fuzzlyMatches(stringToMatch)) {
                            return@mapNotNull section
                        }

                        val filteredPreferences = section.filter { preference ->
                            preference.title.fuzzlyMatches(stringToMatch) ||
                                preference.description.fuzzlyMatches(stringToMatch)
                        }.toTypedArray()
                        ...
                        if (filteredPreferences.isEmpty()) {
                            ...
                            PreferencesSection(section.title, *filteredPreferences)
                        }
                    }
                }
        }
