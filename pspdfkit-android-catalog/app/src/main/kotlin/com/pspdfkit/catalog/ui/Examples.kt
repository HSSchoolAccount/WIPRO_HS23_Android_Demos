...
import com.pspdfkit.catalog.utils.fuzzlyMatches
...
fun Examples(state: State, dispatcher: Dispatcher) {
    ...
    var filteredExamplesInSections by remember { mutableStateOf(listOf<PSPDFExample.Section>()) }
    var dialogVisibility by remember { mutableStateOf(false) }
    var selectedClass by remember { mutableStateOf<PSPDFExample?>(null) }
    ...
    LaunchedEffect(state.examples, state.searchState) {
        withContext(Dispatchers.IO) {
            val stringToMatch = state.searchState
                .searchQueryOrBlank()
            ...
            filteredExamplesInSections =
                if (stringToMatch.isBlank()) {
                    ...
                    state.examples.mapNotNull { section ->

                        // If the section name matches, everything inside of it should match
                        if (section.name.fuzzlyMatches(stringToMatch)) {
                            return@mapNotNull section
                        }

                        val filteredExamplesInSection = section.filter { example ->
                            example.title.fuzzlyMatches(stringToMatch) ||
                                example.exampleName.fuzzlyMatches(stringToMatch) ||
                                example.description.fuzzlyMatches(stringToMatch)
                        }

                        // If there are no examples in a section, we remove said section
                        if (filteredExamplesInSection.isEmpty()) {
                            null
                        } else {
                            PSPDFExample.Section(section.name, section.iconId, filteredExamplesInSection)
                        }
                    }.toList()
                }
        }
    ...
    ExpandableList(
        ...
        itemLayout = { example ->
            val psExample = example as PSPDFExample
            Box(
...
fun ExampleListItem(
    title: String,
    description: String,
    exampleLanguage: PSPDFExample.ExampleLanguage
) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
