...
class MainActivity : AppCompatActivity() {
    ...
    private fun launchExampleWithClassName(exampleClassName: String) {
        try {
            ...
            if (PSPDFExample::class.java.isAssignableFrom(exampleClass)) {
                val constructor: Constructor<*> = exampleClass.getConstructor(Context::class.java)
                (constructor.newInstance(this) as PSPDFExample)
                    .launchExample(this, getPdfActivityConfigurationBuilder())
            } else {
                throw IllegalArgumentException(
                    "Example class " +
                        exampleClassName +
                        " must be assignable to PSPDFExample"
                )
            }
        } catch (ex: Throwable) {
            throw IllegalArgumentException(
                "Can't launch example with class name $exampleClassName",
                ex
            )
        }
    }
