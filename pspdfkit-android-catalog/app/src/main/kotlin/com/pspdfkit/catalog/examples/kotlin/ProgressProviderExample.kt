 ...
 */
class ProgressProviderExample(context: Context) : PSPDFExample(context, R.string.progressProviderExampleTitle, R.string.progressProviderExampleDescription) {
    ...
    private fun startDownloadIfNotRunning(): DownloadJob {
        ...
        if (downloadJob == null) {
            ...
            downloadJob = DownloadJob.startDownload(
                DownloadRequest.Builder(context)
                    .uri(url)
                    .outputFile(File(context.getDir("documents", Context.MODE_PRIVATE), "temp.pdf"))
